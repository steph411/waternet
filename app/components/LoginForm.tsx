...
export const LoginForm:React.FC<Props> = ({className, type = LoginType.signin}) => {
  ...
  const [selectedCity, setSelectedCity] = useState(countriesData[selectedCountry][0]);
    ...
    country: {
      ...
      operation: (data) => {
        if (data) {
          ...
          citySelectRef.current.state.inputValue = countriesData[data?.value][0];
  ...
  const login = async (data) => {
    ...
    const provider = type === LoginType.login ? "credentials-login" : "credentials-signup"
  ...
  return (
    <div className={"w-1/2 px-6 py-8 text-center bg-white shadow-md md:w-full lg:w-1/2 " + className}>
      <div className="flex justify-center">

        <img src="logo_ewater_2.png" className="w-64" />
      </div>
      <h3
        className="py-4 text-lg font-semibold text-center text-light-blue-500"
      >
      ...
      >
                  ...
                  (el.name === "city" || el.name === "country")
                    ...
                    (
                      ...
                      <Select
                        ...
                        options={selectData[el.name].data.map((data) => ({label:data, value: data}))}
                      />
                    )
                    :
        ...
        <p
          className="col-span-2 p-4 text-xs text-center text-light-blue-800"
        >By clicking Sign Up, you agree to the <span className="font-semibold text-light-blue-900">Terms, Data Policy</span> and <span className="font-semibold text-cold-gray-800">Cookies Policy</span> of eWaterGate.</p>
        ...
        <p
          className="col-span-2 text-base font-bold text-center text-light-blue-900"
        >
          {
        ...
        <Link
          href={type === LoginType.login ? "/signup" : "/login"}
        >
          <a
          ...
          > 
            {
              type === LoginType.login ? "New to eWaterGate? Sign Up" : "Already on eWaterGate? Login"
            }
          </a>
        </Link>

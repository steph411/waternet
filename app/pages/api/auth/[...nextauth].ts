  ...
  const options = {
    providers: [
      ...
      Providers.Credentials({
        ...
        name: "signup",
        credentials: {
          firstname: { label: "firstname", type: "text", placeholder: "jsmith" },
    ...
    callbacks: {
      jwt: async (token, user: any, account, profile, isNewUser) => {
        console.log({jwt_callback: {token , user, account, profile, isNewUser}})
        if (user?.id) {
          ...
          if (profile){
            ...
            token = {...token, ...profile}
            const existingUser = await prisma.user.findUnique({where:{id: token.userId}})
            if (!existingUser){
              const newCreatedUser = await prisma.user.create({data: {
                email: token.email,
                name: token.name,
                image: token.picture,
      ...
      session: async (session: any, user: any) => {
        console.log({session_callback: {session, user, usersession: session.user}})

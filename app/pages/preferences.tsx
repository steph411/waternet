...
import {GET_ALL_CATEGORIES, CREATE_USER_CATEGORIES} from '../utils/queries'
...
const Preferences: React.FC<Props> = ({ }) => {
  ...
  const [{fetching, error: mutationError, data: mutationResultData}, createUserCategories] = useMutation(CREATE_USER_CATEGORIES);
  ...
  const onSubmit = async (formData) => {
    ...
    Object.keys(formData).forEach(el => {
      if (formData[el]) {
        mutationData.push( {"category_id": formData[el] , "user_id": session.api["X-Hasura-User-Id"]})
      }
    })
  ...
  const filterByUserCategory = (category, userCategoryId) => {
    return category.user_category && category.user_category.id === userCategoryId;
  }
    ...
    <section className="text-center bg-cold-gray-50">
      <TopSection noImage={true}>
         ...
         <h2 className="w-3/4 pt-40 pb-32 mx-auto text-3xl font-bold text-white lg:w-full">
           eWatergate wants to give you the best experience. Select user and technical categories to connect to the right audience
         </h2>
      </TopSection>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="flex flex-wrap justify-center space-x-4">
          {
            Object.values(userCategoriesObjects).map((userCategory : any, id) => (
              <CategoryList
                ...
                categories={data.categories.filter((category) => filterByUserCategory(category, userCategory.id))}
                register={register}
        ...
        <div className="flex justify-center space-x-4">
              ...
              <CategoryList
                ...
                categories={data.categories.filter((category) => !category.user_category )}
                register={register}
                valueSetter={setValue}
              />
...
const CategoryList: React.FC<CategoryListProps> = ({ title, categories, register, valueSetter}) => {
  ...
  return (
    <div className="flex flex-col mt-4 space-y-4 text-white bg-white" style={{ minWidth: "216px" }}>
      <div
      ...
      {
        categories.map((el, i) => (
          <div key={i} className="flex items-center px-4 py-2 space-x-1 bg-white text-light-blue-900">
            <input
              type="checkbox"
              name={el.name}
              ref={register}

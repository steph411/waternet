...
const FeedsRightSidebar: React.FC<Props> = ({}) => {
  ...
  useClassesBasedOnScroll(waterToolBoxRef, fixedSectionRef, [
    "fixed",
    "top-16",
    "w-80",
  ]);
  ...
  return (
    <section className="self-start col-start-3 col-end-4 space-y-4 overflow-hidden overflow-y-auto lg:hidden w-80">
      <section className="space-y-4" ref={waterToolBoxRef}>
        ...
        <section className="bg-white rounded-lg shadow">
          ...
          <div className="grid grid-cols-3 p-3 gap-x-1">
            {tools.map((el, id) => (
              <div
                key={id}
                className="flex flex-col p-1 cursor-pointer hover:opacity-80 hover:bg-cold-gray-50"
              >
                {<el.logo className="w-10 h-10" />}
                <span className="text-xs font-semibold text-light-blue-800">
                  {el.name}
                </span>
              </div>
            ))}
        ...
        {/* <section className="bg-white rounded-lg shadow">
          ...
          <div className="px-3 pb-2 text-center">
            <h2 className="text-sm font-semibold text-light-blue-900">
              waterpumps
            </h2>
            <h3 className="px-8 text-sm text-light-blue-900">
              Flight N technology water & wastewater pumps
            </h3>
          </div>
        </section> */}
      </section>
      ...
      <section
        ref={fixedSectionRef}
        className="flex flex-col pb-1 bg-white rounded-lg"
      >
        {links.map((el, id) => (
        ...
        <span className="mt-auto text-sm text-center underline text-light-blue-900">
          Copyright © AbundeSEG
        </span>
      </section>
    </section>

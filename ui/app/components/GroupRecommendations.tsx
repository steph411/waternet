...
const GroupRecommendations:React.FC<Props> = ({className, recommendations}) => {
  return (
    <div className={"bg-white rounded py-2 px-4 divide-y divide-cold-gray-400 space-y-4 " + className} >
      <h2 className="font-semibold text-light-blue-900" >Recommended Groups</h2>
      {
        recommendations.map((el,id) => (
          <ProfileRecommendationCard actionMessage="join group" {...el} key={id} />
        ))
      }      

...
const NewPost: React.FC<Props> = ({ onPostCreated, feedType=FeedType.post, textPlaceholder="start a post", actionMessage="publish", update=false, share=false, postId  }) => {
  ...
  return (
    <section className="sticky bg-white rounded-lg shadow-md ring-2 ring-light-blue-900">
      ...
      {/* uploaded media section */}
      {
        file && uploadedImage.url && (
          <div className="relative flex-none px-8 my-2 overflow-hidden rounded-sm">  
            <span 
              onClick={() => {
                setFile(null);
                setUploadedImage({name:"", url:""})
              }} 
              className="absolute top-0 px-2 py-1 font-semibold text-red-300 transition-all bg-white rounded-full shadow-sm cursor-pointer hover:text-red-400 hover:shadow left-1">x</span>
            {
